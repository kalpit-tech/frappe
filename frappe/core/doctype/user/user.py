...
class User(Document):
    ...
    def on_update(self):
        ...
        if self.name not in ('Administrator', 'Guest') and not self.user_image:
            frappe.enqueue(
                'frappe.core.doctype.user.user.update_gravatar', name=self.name)
    ...
    def check_demo(self):
        if frappe.session.user == 'demo@erpnext.com':
            frappe.throw(
                _('Cannot change user details in demo. Please signup for a new account at https://erpnext.com'), title=_('Not Allowed'))
    ...
    def validate_share(self, docshare):
        if docshare.user == self.name:
            if self.user_type == "System User":
                if docshare.share != 1:
                    frappe.throw(
                        _("Sorry! User should have complete access to their own record."))
            else:
                frappe.throw(
                    _("Sorry! Sharing with Website User is prohibited."))
        ...
        try:
            if self.flags.in_insert:
                if self.name not in STANDARD_USERS:
                    ...
                    if not self.flags.no_welcome_mail and self.send_welcome_email and not self.flags.ignore_welcome_mail_to_user:
    ...
    def send_login_mail(self, subject, template, add_args, now=None):
        ...
        frappe.sendmail(recipients=self.email, sender=sender, subject=subject,
                        template=template, args=args, header=[
                            subject, "green"],
    ...
    def on_trash(self):
        ...
        # delete todos
        frappe.db.sql(
            """DELETE FROM `tabToDo` WHERE `owner`=%s""", (self.name,))
    ...
    def after_rename(self, old_name, new_name, merge=False):
        ...
        if frappe.db.exists("Chat Profile", old_name):
            frappe.rename_doc("Chat Profile", old_name,
    ...
    def remove_all_roles_for_guest(self):
        if self.name == "Guest":
            self.set("roles", list(
                set(d for d in self.get("roles") if d.role == "Guest")))
    ...
    def suggest_username(self):
        ...
        if not username:
            # @firstname_last_name
            username = _check_suggestion(frappe.scrub(
                "{0} {1}".format(self.first_name, self.last_name or "")))
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing):
    ...
    user_names = frappe.db.get_values(
        "User", {"email": email_id}, as_dict=True)
    if not user_names:
        return
    ...
    if udpate_user_email_settings:
        ...
        users = " and ".join([frappe.bold(user.get("name"))
                              for user in user_names])
...
def sign_up(email, full_name, user_type, brand_name, redirect_to):
    ...
    if user:
        ...
        if(user_type == 'brand'):
            if(isBrandNameExists(brand_name)):
                return 0, _("Brand Name Already Exists")
...
def isBrandNameExists(brand_name):
    existing_brands = frappe.get_all(
        "User", filters={'brand_name': brand_name})
...
def send_token_via_email(tmp_id, token=None):
    ...
    frappe.sendmail(
        ...
        message='<p>Your verification code is {0}</p>'.format(
            hotp.at(int(count))),
...
def reset_otp_secret(user):
    ...
    if frappe.session.user in ["Administrator", user]:
        ...
        enqueue(method=frappe.sendmail, queue='short', timeout=300,
                event=None, is_async=True, job_name=None, now=False, **email_args)
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
    ...
    if not contact_exists:
        ...
        contact.insert(ignore_permissions=True,
                       ignore_links=ignore_links, ignore_mandatory=ignore_mandatory)
    else:
